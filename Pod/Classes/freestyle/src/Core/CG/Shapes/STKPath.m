...
//
<<<<<<<< HEAD:Pod/Classes/freestyle/src/Core/CG/Shapes/STKPXPath.m
//  STKPXPath.m
========
//  STKPath.m
>>>>>>>> 27bba7ae5b4ef1d48809e50f4d67780e15e00e9a:Pod/Classes/freestyle/src/Core/CG/Shapes/STKPath.m
//  Pixate
//
...
{
    STKPXPath *path = [[STKPXPath alloc] init];
========
...
- (void)ellipticalArcRadiusX:(CGFloat)rx radiusY:(CGFloat)ry xAxisRotation:(CGFloat)xAxisRotation largeArcFlag:(BOOL)largeArcFlag sweepFlag:(BOOL)sweepFlag x:(CGFloat)x y:(CGFloat)y
{
    ...
    if ( rx != 0.0 || ry != 0.0 )
    {
        ...
        STKPXVector *u = [STKPXVector vectorWithX:1.0 Y:0.0];
        // NOTE: SVG spec divides x-component by rx and y-component by ry
        STKPXVector *v = [STKPXVector vectorWithX:(x1p - cxp) Y:(y1p - cyp)];
        STKPXVector *w = [STKPXVector vectorWithX:(-x1p - cxp) Y:(-y1p - cyp)];
