...
@implementation STKPXNumberMatcher
{
...
- (STKPXStylesheetLexeme *)createLexemeWithString:(NSString *)aString withRange:(NSRange)aRange
{
    ...
    if (matches.count == 1)
    {
        ...
        if (!NSEqualRanges(dimensionRange, NO_MATCH))
        {
            ...
            STKPXDimension *dimensionValue = [STKPXDimension dimensionWithNumber:floatValue withDimension:dimension];

            result = [STKPXStylesheetLexeme lexemeWithType:type withRange:matchResult.range withValue:dimensionValue];

        }
        else
        {
            result = [STKPXStylesheetLexeme lexemeWithType:self.type withRange:numberRange withValue:@(floatValue)];
        }
    }
