...
@implementation STKPXURLMatcher
...
- (STKPXStylesheetLexeme *)createLexemeWithString:(NSString *)aString withRange:(NSRange)aRange
{
    ...
    if (match)
    {
        for (NSUInteger i = 1; i < match.numberOfRanges; i++)
        {
            ...
            {
                result = [STKPXStylesheetLexeme lexemeWithType:self.type withRange:match.range withValue:[aString substringWithRange:stringRange]];
                break;
            }
        }
    }
