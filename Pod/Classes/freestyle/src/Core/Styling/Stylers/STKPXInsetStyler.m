...
@implementation STKPXInsetStyler
{
...
- (NSDictionary *)declarationHandlers
{
    ...
    {
        result = @{
            _shortcutName : ^(STKPXDeclaration *declaration, STKPXStylerContext *context) {
                self.insets = declaration.insetsValue;
            },
            _topName : ^(STKPXDeclaration *declaration, STKPXStylerContext *context) {
            ...
            _rightName: ^(STKPXDeclaration *declaration, STKPXStylerContext *context) {
                UIEdgeInsets insets = context.insets;
                CGFloat value = declaration.floatValue;

                self.insets = UIEdgeInsetsMake(insets.top, insets.left, insets.bottom, value);
            },
            _bottomName: ^(STKPXDeclaration *declaration, STKPXStylerContext *context) {
                UIEdgeInsets insets = context.insets;
                CGFloat value = declaration.floatValue;

                self.insets = UIEdgeInsetsMake(insets.top, insets.left, value, insets.right);
            },
            _leftName : ^(STKPXDeclaration *declaration, STKPXStylerContext *context) {
                UIEdgeInsets insets = context.insets;
                CGFloat value = declaration.floatValue;
