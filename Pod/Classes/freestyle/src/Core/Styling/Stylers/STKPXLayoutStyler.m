...
@implementation STKPXLayoutStyler

#pragma mark - Static Methods

+ (STKPXLayoutStyler *)sharedInstance
{
	static __strong STKPXLayoutStyler *sharedInstance = nil;
	static dispatch_once_t onceToken;

	dispatch_once(&onceToken, ^{
		sharedInstance = [[STKPXLayoutStyler alloc] init];
	});
...
- (NSDictionary *)declarationHandlers
{
    ...
    dispatch_once(&onceToken, ^{
        handlers = @{
            @"top" : ^(STKPXDeclaration *declaration, STKPXStylerContext *context) {
                context.top = declaration.floatValue;
            },
            @"left" : ^(STKPXDeclaration *declaration, STKPXStylerContext *context) {
                context.left = declaration.floatValue;
            },
            @"width" : ^(STKPXDeclaration *declaration, STKPXStylerContext *context) {
                context.width = declaration.floatValue;
            },
            @"height" : ^(STKPXDeclaration *declaration, STKPXStylerContext *context) {
                context.height = declaration.floatValue;
            },
            @"size" : ^(STKPXDeclaration *declaration, STKPXStylerContext *context) {
                context.width = declaration.sizeValue.width;
                context.height = declaration.sizeValue.height;
            },
            @"position" : ^(STKPXDeclaration *declaration, STKPXStylerContext *context) {
